...
if [[ $# -ne 0 ]]; then
  die "in-container Debian 13 aardvark-dns build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-aardvark-dns-debian-trixie"
...
prepare_sources() {
  ...
  log "Fetching Debian ${DISTRO} aardvark-dns source package"
  apt-get source -qq aardvark-dns
  DEBIAN_SRC_DIR="$(find "${WORK_ROOT}" -maxdepth 1 -mindepth 1 -type d -name 'aardvark-dns-*' | head -n 1)"
  [[ -n "${DEBIAN_SRC_DIR}" ]] || die "unable to locate unpacked Debian aardvark-dns source directory"
...
update_changelog() {
  ...
  export DEBFULLNAME="Aardvark-DNS Debian 13 Builder"
  ...
  dch \
    ...
    "Build upstream aardvark-dns ${AARDVARK_TAG} (${build_id}) with Debian ${DISTRO} packaging and repo-managed patch series."
}
...
main() {
  log "Starting Debian 13 containerized aardvark-dns build for ${TARGET_ARCH} (${AARDVARK_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  install_rust_toolchain
  ...
  build_package
  collect_artifacts
  log "Completed Debian 13 aardvark-dns build for ${TARGET_ARCH} (${AARDVARK_TAG})"
}
