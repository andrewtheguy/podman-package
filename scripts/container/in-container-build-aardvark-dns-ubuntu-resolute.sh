...
if [[ $# -ne 0 ]]; then
  die "in-container Ubuntu 26.04 aardvark-dns build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-aardvark-dns-ubuntu-resolute"
...
update_changelog() {
  ...
  export DEBFULLNAME="Aardvark-DNS Resolute Builder"
...
main() {
  log "Starting Ubuntu 26.04 containerized aardvark-dns build for ${TARGET_ARCH} (${AARDVARK_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  install_rust_toolchain
  ...
  build_package
  collect_artifacts
  log "Completed Ubuntu 26.04 aardvark-dns build for ${TARGET_ARCH} (${AARDVARK_TAG})"
}
