...
if [[ $# -ne 0 ]]; then
  die "in-container Ubuntu 24.04 containers-common build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-containers-common-ubuntu-noble"
...
update_changelog() {
  ...
  export DEBFULLNAME="Containers-Common Noble Builder"
...
main() {
  log "Starting Ubuntu 24.04 containerized containers-common build for ${TARGET_ARCH} (${CONTAINERS_COMMON_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  patch_debian_packaging
  ...
  build_package
  collect_artifacts
  log "Completed Ubuntu 24.04 containers-common build for ${TARGET_ARCH} (${CONTAINERS_COMMON_TAG})"
}
