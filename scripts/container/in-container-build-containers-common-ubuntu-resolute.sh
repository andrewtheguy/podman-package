...
if [[ $# -ne 0 ]]; then
  die "in-container Ubuntu 26.04 containers-common build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-containers-common-ubuntu-resolute"
...
prepare_sources() {
  ...
  log "Fetching Ubuntu ${DISTRO} golang-github-containers-common source package"
  ...
  [[ -n "${DEBIAN_SRC_DIR}" ]] || die "unable to locate unpacked Ubuntu containers-common source directory"
...
update_changelog() {
  ...
  export DEBFULLNAME="Containers-Common Resolute Builder"
  ...
  dch \
    ...
    "Build upstream containers-common ${CONTAINERS_COMMON_TAG} (${build_id}) with Ubuntu ${DISTRO} packaging and repo-managed patch series."
}
...
main() {
  log "Starting Ubuntu 26.04 containerized containers-common build for ${TARGET_ARCH} (${CONTAINERS_COMMON_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  patch_debian_packaging
  ...
  build_package
  collect_artifacts
  log "Completed Ubuntu 26.04 containers-common build for ${TARGET_ARCH} (${CONTAINERS_COMMON_TAG})"
}
