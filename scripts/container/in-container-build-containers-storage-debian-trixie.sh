...
if [[ $# -ne 0 ]]; then
  die "in-container Debian 13 containers-storage build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-containers-storage-debian-trixie"
...
prepare_sources() {
  ...
  log "Fetching Debian ${DISTRO} golang-github-containers-storage source package"
  ...
  [[ -n "${DEBIAN_SRC_DIR}" ]] || die "unable to locate unpacked Debian containers-storage source directory"
...
update_changelog() {
  ...
  export DEBFULLNAME="Containers-Storage Debian 13 Builder"
  ...
  dch \
    ...
    "Build upstream containers-storage ${CONTAINERS_STORAGE_TAG} (${build_id}) with Debian ${DISTRO} packaging and repo-managed patch series."
}
...
main() {
  log "Starting Debian 13 containerized containers-storage build for ${TARGET_ARCH} (${CONTAINERS_STORAGE_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  derive_go_version
  ...
  build_package
  collect_artifacts
  log "Completed Debian 13 containers-storage build for ${TARGET_ARCH} (${CONTAINERS_STORAGE_TAG})"
}
