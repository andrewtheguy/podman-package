...
if [[ $# -ne 0 ]]; then
  die "in-container Ubuntu 24.04 containers-storage build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-containers-storage-ubuntu-noble"
...
update_changelog() {
  ...
  export DEBFULLNAME="Containers-Storage Noble Builder"
...
main() {
  log "Starting Ubuntu 24.04 containerized containers-storage build for ${TARGET_ARCH} (${CONTAINERS_STORAGE_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  derive_go_version
  ...
  build_package
  collect_artifacts
  log "Completed Ubuntu 24.04 containers-storage build for ${TARGET_ARCH} (${CONTAINERS_STORAGE_TAG})"
}
