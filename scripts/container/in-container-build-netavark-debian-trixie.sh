...
if [[ $# -ne 0 ]]; then
  die "in-container Debian 13 netavark build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-netavark-debian-trixie"
...
prepare_sources() {
  ...
  log "Fetching Debian ${DISTRO} netavark source package"
  apt-get source -qq netavark
  DEBIAN_SRC_DIR="$(find "${WORK_ROOT}" -maxdepth 1 -mindepth 1 -type d -name 'netavark-*' | head -n 1)"
  [[ -n "${DEBIAN_SRC_DIR}" ]] || die "unable to locate unpacked Debian netavark source directory"
...
update_changelog() {
  ...
  export DEBFULLNAME="Netavark Debian 13 Builder"
  ...
  dch \
    ...
    "Build upstream netavark ${NETAVARK_TAG} (${build_id}) with Debian ${DISTRO} packaging and repo-managed patch series."
}
...
main() {
  log "Starting Debian 13 containerized netavark build for ${TARGET_ARCH} (${NETAVARK_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  install_rust_toolchain
  ...
  build_package
  collect_artifacts
  log "Completed Debian 13 netavark build for ${TARGET_ARCH} (${NETAVARK_TAG})"
}
