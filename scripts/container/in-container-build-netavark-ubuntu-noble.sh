...
if [[ $# -ne 0 ]]; then
  die "in-container Ubuntu 24.04 netavark build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-netavark-ubuntu-noble"
...
update_changelog() {
  ...
  export DEBFULLNAME="Netavark Noble Builder"
...
main() {
  log "Starting Ubuntu 24.04 containerized netavark build for ${TARGET_ARCH} (${NETAVARK_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  install_rust_toolchain
  ...
  build_package
  collect_artifacts
  log "Completed Ubuntu 24.04 netavark build for ${TARGET_ARCH} (${NETAVARK_TAG})"
}
