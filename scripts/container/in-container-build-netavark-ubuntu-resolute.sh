...
if [[ $# -ne 0 ]]; then
  die "in-container Ubuntu 26.04 netavark build script does not accept arguments"
fi
...
: "${BUILD_REVISION:=1}"
: "${NETAVARK_UPSTREAM_SHA256:?NETAVARK_UPSTREAM_SHA256 is required}"
: "${NETAVARK_VENDOR_SHA256:?NETAVARK_VENDOR_SHA256 is required}"
: "${RUST_VERSION:?RUST_VERSION is required}"
: "${DISTRO:=resolute}"

[[ "${BUILD_REVISION}" =~ ^[1-9][0-9]*$ ]] || die "BUILD_REVISION must be a positive integer: ${BUILD_REVISION}"

PATCH_SOURCE_DIR="/workspace/packaging/patches-netavark-ubuntu-resolute"
...
prepare_sources() {
  ...
  log "Fetching Ubuntu ${DISTRO} netavark source package"
  apt-get source -qq netavark
  DEBIAN_SRC_DIR="$(find "${WORK_ROOT}" -maxdepth 1 -mindepth 1 -type d -name 'netavark-*' | head -n 1)"
  [[ -n "${DEBIAN_SRC_DIR}" ]] || die "unable to locate unpacked Ubuntu netavark source directory"

  local upstream_version="${NETAVARK_TAG#v}"
  UPSTREAM_VERSION="${upstream_version}"
  local upstream_tarball="${WORK_ROOT}/netavark-${upstream_version}.tar.gz"
  local upstream_url="https://github.com/containers/netavark/archive/refs/tags/${NETAVARK_TAG}.tar.gz"
  log "Downloading upstream netavark source: ${upstream_url}"
  curl -fsSL -o "${upstream_tarball}" -L "${upstream_url}"
  verify_sha256 "${upstream_tarball}" "${NETAVARK_UPSTREAM_SHA256}"
  tar -xzf "${upstream_tarball}" -C "${WORK_ROOT}"

  UPSTREAM_SRC_DIR="${WORK_ROOT}/netavark-${upstream_version}"
  ...
  local vendor_tarball="${WORK_ROOT}/netavark-${NETAVARK_TAG}-vendor.tar.gz"
  local vendor_url="https://github.com/containers/netavark/releases/download/${NETAVARK_TAG}/netavark-${NETAVARK_TAG}-vendor.tar.gz"
  log "Downloading netavark vendored dependencies: ${vendor_url}"
  curl -fsSL -o "${vendor_tarball}" -L "${vendor_url}"
  verify_sha256 "${vendor_tarball}" "${NETAVARK_VENDOR_SHA256}"
...
patch_debian_rules() {
  ...
  # cannot satisfy netavark's modern Cargo.lock. Build with the upstream Makefile
  # against the vendored crates and our own pinned Rust toolchain instead.
...
# (debian/netavark/usr/bin) and relocates it; our make-based install already
# stages the final /usr/lib/podman layout, so neutralize the hook.
execute_after_dh_install:
	true

# Distro packaging may ship a narrower file list than upstream `make install`.
# Do not fail the build on staged-but-uninstalled files; debian/netavark.install
# captures everything we intend to ship.
override_dh_missing:
	dh_missing
EOF_MODULE_BUILD
}
...
patch_debian_packaging() {
  ...
  # Install exactly what upstream `make install` produces for this tag,
  # decoupled from the (older) distro .install layout.
...
update_changelog() {
  ...
  export DEBFULLNAME="Netavark Resolute Builder"
  ...
  dch \
    ...
    "Build upstream netavark ${NETAVARK_TAG} (${build_id}) with Ubuntu ${DISTRO} packaging and repo-managed patch series."
}
...
main() {
  log "Starting Ubuntu 26.04 containerized netavark build for ${TARGET_ARCH} (${NETAVARK_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  install_rust_toolchain
  ...
  build_package
  collect_artifacts
  log "Completed Ubuntu 26.04 netavark build for ${TARGET_ARCH} (${NETAVARK_TAG})"
}
