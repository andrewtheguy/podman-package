...
if [[ $# -ne 0 ]]; then
  die "in-container Ubuntu 26.04 build script does not accept arguments"
fi
...
PATCH_SOURCE_DIR="/workspace/packaging/patches-ubuntu-resolute"
...
prepare_sources() {
  ...
  log "Fetching Ubuntu ${DISTRO} podman source package"
  apt-get source -qq podman
  UBUNTU_SRC_DIR="$(find "${WORK_ROOT}" -maxdepth 1 -mindepth 1 -type d -name 'podman-*' | head -n 1)"
  [[ -n "${UBUNTU_SRC_DIR}" ]] || die "unable to locate unpacked Ubuntu podman source directory"
...
update_changelog() {
  ...
  export DEBFULLNAME="Podman Resolute Builder"
...
build_package() {
  ...
  log "Applying patch series from /workspace/packaging/patches-ubuntu-resolute/series"
...
main() {
  log "Starting Ubuntu 26.04 containerized build for ${TARGET_ARCH} (${PODMAN_TAG})"
  setup_sources
  install_prereqs
  prepare_sources
  install_go_toolchain
  ...
  build_package
  collect_artifacts
  log "Completed Ubuntu 26.04 build for ${TARGET_ARCH} (${PODMAN_TAG})"
}
