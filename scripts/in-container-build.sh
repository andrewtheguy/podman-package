...
OUT_DIR="/out/${PODMAN_TAG}/${TARGET_ARCH}"
...
update_changelog() {
  ...
  local package_version="${debianized_upstream}+local1~${DISTRO}"
  ...
  dch \
    ...
    "Build upstream ${PODMAN_TAG} with Ubuntu ${DISTRO} packaging and repo-managed patch series."
}
